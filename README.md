...
#### **Лабораторна робота №7.** <-- Доробити
##### > Паттерн Зберігач застосований для надання можливості користувачам виконувати пошук таблеток у магазині(за виробником, назвою, ціною та типом, використовуючи логічні "І" та "Або").
_Коли застосовується для часто повторюваних операцій і треба визначити власний синтаксис_
>- Memento: охоронець, який зберігає стан об'єкта Originator і надає повний доступ тільки цього об'єкту Originator.
>- Originator: створює об'єкт зберігача для збереження свого стану.
...
##### > Паттерн Відвідувач застосований для оплати замовлення, яке є у кошику(PayPal або кредитна картка).
_Коли є безліч взаємопов'язаність об'єктів, зв'язки між якими складні і заплутані_
>- Visitor: інтерфейс користувача, який визначає метод Visit () для кожного об'єкта Element
>- ConcreteVisitor1 / ConcreteVisitor2: конкретні класи відвідувачів, реалізують інтерфейс, визначений в Visitor.
>- Element: визначає метод Accept (), в якому в якості параметра приймається об'єкт Visitor.
>- ElementA / ElementB: конкретні елементи, які реалізують метод Accept ().
>- ObjectStructure: деяка структура, яка зберігає об'єкти Element і надає до них доступ. Це можуть бути і прості списки, і складні складові структури у вигляді дерев.
